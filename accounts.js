...
function isDateReady(value) {
  const dateKey = getDateKey(value);
  return Boolean(dateKey && dateKey <= formatDateKey(new Date()));
}
...
function getDealReceivedAmount(deal = {}) {
  ...
  const explicitValue = explicitValues.find(
  ...
  return getStatusValue(deal.pay_stat) === "received"
    ? Number(deal.deal_amount || 0)
    : 0;
}

function getDealPaymentDate(deal = {}) {
  return deal.payment_date || deal.closed_date || "";
}

function hasReceivedPayment(deal = {}) {
  return getDealReceivedAmount(deal) > 0 && Boolean(getDealPaymentDate(deal));
}
...
function getDealsFilters() {
  return {
    search: String(document.getElementById("collectionsSearch")?.value || "")
      ...
      .toLowerCase(),
    method: String(
      document.getElementById("collectionsMethodFilter")?.value || "",
    )
      .trim()
      .toLowerCase(),
  };
...
function updateMethodFilterOptions() {
  ...
  const methods = [
    ...new Set(
      accountsDeals.map((deal) => normalizePaymentMethod(deal.payment_method)),
    ),
  ];
...
function renderDashboard() {
  ...
  const receivedDeals = accountsDeals.filter(
    (deal) => getStatusValue(deal.pay_stat) === "received",
  );
  const pendingDeals = accountsDeals.filter(
    (deal) => getStatusValue(deal.pay_stat) === "pending",
  );
  const failedDeals = accountsDeals.filter(
    (deal) => getStatusValue(deal.pay_stat) === "failed",
  );
  ...
  const collectionRate = totalDeals
    ? Math.round((receivedDeals.length / totalDeals) * 100)
    : 0;
  ...
  document.getElementById("receivedRevenueMetric").textContent =
    formatCurrency(receivedRevenue);
  document.getElementById("receivedRevenueHint").textContent =
    `${receivedDeals.length} deals received`;
  document.getElementById("pendingRevenueMetric").textContent =
    formatCurrency(pendingRevenue);
  document.getElementById("pendingRevenueHint").textContent =
    `${pendingDeals.length} deals need follow-up`;
  document.getElementById("invoiceReadyMetric").textContent = String(
    receivedDeals.length,
  );
  document.getElementById("invoiceReadyHint").textContent =
    `${receivedDeals.length} tax invoices can go out`;
  document.getElementById("failedPaymentsMetric").textContent = String(
    failedDeals.length,
  );
  document.getElementById("failedPaymentsHint").textContent =
    `${failedDeals.length} recovery cases open`;

  document.getElementById("closedDealsCount").textContent = String(totalDeals);
  document.getElementById("receivedDealsCount").textContent = String(
    receivedDeals.length,
  );
  document.getElementById("pendingDealsCount").textContent = String(
    pendingDeals.length,
  );
  document.getElementById("failedDealsCount").textContent = String(
    failedDeals.length,
  );
  document.getElementById("collectionFunnelRate").textContent =
    `${realizedRevenueRate}% realized revenue`;
  document.getElementById("collectionRateValue").textContent =
    `${collectionRate}%`;
  ...
  const priorityRows = [...failedDeals, ...pendingDeals]
    .sort((left, right) => {
      const leftDate = new Date(
        left.payment_date || left.closed_date || left.created_at || 0,
      ).getTime();
      const rightDate = new Date(
        right.payment_date || right.closed_date || right.created_at || 0,
      ).getTime();
...
function renderCollectionsMiniKpis(rows) {
  ...
  const visibleAmount = rows.reduce(
    (sum, deal) => sum + Number(deal.deal_amount || 0),
    0,
  );
  const proofCount = rows.filter((deal) =>
    buildPaymentProofUrl(deal.payment_proof),
  ).length;
...
async function refreshAccountsPanel() {
  try {
    ...
    showPopup(
      "Accounts",
      error.message || "Failed to refresh ACC dashboard",
      false,
    );
  }
}
...
async function shareProformaInvoiceWhatsApp(id, phone) {
  ...
  try {
    ...
    showPopup(
      "Invoice Center",
      error.message || "Failed to share proforma invoice PDF",
      false,
    );
  }
}
...
async function shareTaxInvoiceWhatsApp(id, phone) {
  ...
  try {
    ...
    showPopup(
      "Invoice Center",
      error.message || "Failed to share tax invoice PDF",
      false,
    );
  }
}
...
async function shareProformaInvoiceGmail(email, id) {
  ...
  try {
    ...
    showPopup(
      "Invoice Center",
      result.message || "Proforma invoice emailed successfully",
      true,
    );
  } catch (error) {
    ...
    showPopup(
      "Invoice Center",
      error.message || "Failed to email proforma invoice PDF",
      false,
    );
  }
}
...
async function shareTaxInvoiceGmail(email, id) {
  ...
  try {
    ...
    showPopup(
      "Invoice Center",
      result.message || "Tax invoice emailed successfully",
      true,
    );
  } catch (error) {
    ...
    showPopup(
      "Invoice Center",
      error.message || "Failed to email tax invoice PDF",
      false,
    );
  }
}
...
function bindCollectionFilters() {
  ...
  document
    .getElementById("collectionsSearch")
    ?.addEventListener("input", rerender);
  document
    .getElementById("collectionsMethodFilter")
...
function getCurrentLocation() {
  return new Promise((resolve, reject) => {
    if (!window.isSecureContext) {
      reject(
        new Error("Location ke liye page localhost ya HTTPS par open karo."),
      );
      return;
    }
