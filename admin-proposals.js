(() => {
  ...
  function getCurrentCompanyScope() {
    ...
    return (
      normalizeProposalCompanyKey(
        localStorage.getItem("adminSelectedCompanyScope") ||
          admin.company_key ||
  ...
  function withCompanyScope(url) {
    ...
    const encodedScope = encodeURIComponent(companyScope);
    return `${url}${separator}companyScope=${encodedScope}&company_scope=${encodedScope}&company=${encodedScope}`;
  }
  ...
  function normalizeEditorContent(value) {
    // value may be HTML — strip tags for plain-text canvas rendering
  ...
  function buildCanvasLines(ctx, proposal = {}, contentWidth) {
    ...
    const segments = rawHtml
      .replace(/<hr\s*\/?>/gi, "||HR||")
      .replace(/<h2([^>]*)>([\s\S]*?)<\/h2>/gi, (_, _a, content) => `||H2||${htmlToPlainText(content)}||END||`)
      .replace(/<h3([^>]*)>([\s\S]*?)<\/h3>/gi, (_, _a, content) => `||H3||${htmlToPlainText(content)}||END||`)
      .replace(/<p([^>]*)>([\s\S]*?)<\/p>/gi, (_, _a, content) => `||P||${htmlToPlainText(content)}||END||`)
      .replace(/<strong([^>]*)>([\s\S]*?)<\/strong>/gi, (_, _a, content) => htmlToPlainText(content))
      .replace(/<[^>]+>/g, " ")
      .split("||");
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      const plain = htmlToPlainText(tag);
      if (plain && plain !== "END") {
        plain.split("\n").forEach((line) => {
