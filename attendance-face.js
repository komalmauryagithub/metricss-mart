(function () {
  ...
  function buildSignatureFromImageData(imageData) {
    ...
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        ...
        histogram[
          HISTOGRAM_BUCKETS + Math.min(HISTOGRAM_BUCKETS - 1, Math.floor(green / 64))
        ] += 1;
        histogram[
          HISTOGRAM_BUCKETS * 2 +
            Math.min(HISTOGRAM_BUCKETS - 1, Math.floor(blue / 64))
        ] += 1;
    ...
    const average =
      cellAverages.reduce((sum, value) => sum + value, 0) /
      Math.max(cellAverages.length, 1);
    const pixelCount = width * height;
    const normalizedHistogram = histogram.map((value) =>
      Number((value / pixelCount).toFixed(6)),
    );
  ...
  async function captureAttendancePayload(options = {}) {
    const capture = await captureForAttendance(options);
    return {
      attendance_face_image: capture.faceImage,
      attendance_face_signature: JSON.stringify(capture.faceSignature),
      faceImage: capture.faceImage,
      faceSignature: capture.faceSignature,
    };
  }
