...
function disableForm(message, type = "error") {
  ...
  if (type === "error") {
    ...
    const inputs = profileForm.querySelectorAll("input, select, textarea, button");
...
async function loadProfileForm() {
  ...
  try {
    ...
    const response = await fetch(
      `${PROFILE_BASE_URL}/api/me/${encodeURIComponent(profileUserId)}`,
      {
        cache: "no-store",
      },
    );
    ...
    if (!response.ok || !result.success || !userData) {
    ...
    if (userData.profile_setup_status === "completed") {
...
async function handleProfileSubmit(event) {
  ...
  try {
    ...
    setStatusBadge("completed");
    setStatusBadge("completed");
    disableForm(result.message || "Profile details submitted successfully.", "success");
    setFormMessage("");
    
    try {
      const meRes = await fetch(`${PROFILE_BASE_URL}/api/me/${encodeURIComponent(profileUserId)}`);
      const meJson = await meRes.json();
      if (meJson.success && meJson.user) {
        let current = JSON.parse(localStorage.getItem('currentUser') || '{}');
        localStorage.setItem('currentUser', JSON.stringify({...current, ...meJson.user}));
      }
    } catch(e) {
