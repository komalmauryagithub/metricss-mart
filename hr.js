...
let hrAttendanceUpdating = false;
...
function setupToolbar() {
  ...
  const statusFilter = document.getElementById("hrEmployeeStatusFilter");
  ...
  statusFilter?.addEventListener("change", () => handleHrEmployeeStatusFilterChange());
}
...
function normalizeHrCompanyScopeKey(value) {
...
function getHrCompanyScopeKey() {
  return (
    normalizeHrCompanyScopeKey(
      currentUser?.company_key ||
        currentUser?.companyScope ||
...
function getHrCompanyScopeQuery() {
  return `&companyScope=${encodeURIComponent(getHrCompanyScopeKey())}`;
}
...
function normalizeHrEmploymentStatusFilter(value, fallback = "active") {
  const normalized = String(value || "")
    .toLowerCase()
    .trim()
    .replace(/[\s-]+/g, "_");
...
function getHrEmployeeStatusFilter() {
  const filter = normalizeHrEmploymentStatusFilter(
    document.getElementById("hrEmployeeStatusFilter")?.value ||
      hrState.employeeStatusFilter,
  );
  const select = document.getElementById("hrEmployeeStatusFilter");
  if (select && select.value !== filter) select.value = filter;
  return filter;
}

function isHrEmployeeActive(employee = {}) {
  return normalizeHrEmploymentStatusFilter(
    employee.employment_status || employee.employmentStatus,
  ) === "active";
}

function handleHrEmployeeStatusFilterChange() {
  hrState.employeeStatusFilter = getHrEmployeeStatusFilter();
  refreshHrPanel({ showSuccessToast: false });
}
...
async function refreshHrPanel(options = {}) {
  ...
  const employeeStatusFilter = getHrEmployeeStatusFilter();
  const payrollMonth = normalizeMonthInput(
    document.getElementById("hrMonthPicker")?.value || "",
  );
  const companyScopeQuery = getHrCompanyScopeQuery();
  ...
  try {
    const [employeesResult, attendanceResult, leavesResult, payrollResult] =
      await Promise.all([
        fetchHrEmployeesData(currentUser.id, employeeStatusFilter),
        fetchJson(
          `${BASE_URL}/api/admin/attendance?companyScope=${encodeURIComponent(getHrCompanyScopeKey())}`,
        ),
    ...
    const [
      ...
      fetchOptionalJson(`${BASE_URL}/api/admin/team-report?employmentStatus=${encodeURIComponent(employeeStatusFilter)}${companyScopeQuery}`, { data: [] }),
    ...
    const payrollRows = Array.isArray(payrollResult.data)
      ? payrollResult.data.filter((row) => normalizeRole(row.role) !== "admin")
      : [];
...
function applyEmployeeSearchFilter() {
  const query = String(document.getElementById("hrEmployeeSearch")?.value || "")
    .trim()
    .toLowerCase();
  const statusFilter = normalizeHrEmploymentStatusFilter(hrState.employeeStatusFilter);

  hrState.filteredEmployees = hrState.employees.filter((employee) => {
    if (
      statusFilter !== "all" &&
      normalizeHrEmploymentStatusFilter(employee.employment_status) !== statusFilter
    ) {
      return false;
    }
...
function formatHrAttendanceTime(value) {
...
function getHrAttendanceTimeSeconds(value) {
...
function isInvalidHrAttendanceCheckout(row) {
  const checkOutRaw = String(row?.check_out || "").trim();
  if (/^00:00(?::00)?$/.test(checkOutRaw)) return true;

  const checkInSeconds = getHrAttendanceTimeSeconds(row?.check_in);
  const checkOutSeconds = getHrAttendanceTimeSeconds(checkOutRaw);
  return checkInSeconds !== null &&
    checkOutSeconds !== null &&
    checkOutSeconds < checkInSeconds;
}

function hasValidHrAttendanceCheckout(row) {
  return Boolean(row?.check_out) && !isInvalidHrAttendanceCheckout(row);
}
...
function renderHrEmployeeProfileDetails(employee = {}) {
  ...
  const accessStatus = isHrEmployeeActive(employee) ? "Active" : "Deactive";
  ...
  body.innerHTML = `
    ...
    <div class="profile-detail-grid">
      ${renderHrProfileSection("Admin Entered Account Details", [
        ...
        { label: "Deactivated at", value: formatHrProfileDateTime(employee.deactivated_at) },
        { label: "Deactivated by", value: employee.deactivated_by_name },
...
function getEmployeeLifecycleStatus(employee) {
  if (!isHrEmployeeActive(employee)) {
    return { label: "Deactive", tone: "bad" };
  }
  ...
  const attendanceStatus = normalizeHrAttendanceStatus(employee?.attendance_status);
  if (["present", "grace", "late", "checkout_pending"].includes(attendanceStatus)) {
    return { label: "Active", tone: "good" };
  }

  if (attendanceStatus === "half_day" || attendanceStatus === "half_day_late") {
...
function normalizeHrAttendanceStatus(status) {
...
function getAttendanceStatusLabel(status) {
  switch (normalizeHrAttendanceStatus(status)) {
    case "present":
      return "Present";
    case "grace":
      return "Grace";
...
function getStatusTone(status) {
  const normalized = normalizeHrAttendanceStatus(status);
  if (["present", "grace", "sunday", "approved", "active", "ready"].includes(normalized)) return "good";
  if (["late", "half_day", "half_day_late", "pending", "scheduled", "warning"].includes(normalized)) return "warn";
...
function mapLegacyUserToHrEmployee(user) {
  ...
  return {
    ...
    department:
      ...
      "General",
    employment_status: normalizeHrEmploymentStatusFilter(user?.employment_status),
...
async function fetchHrEmployeesData(userId, employmentStatus = "active") {
  const statusFilter = normalizeHrEmploymentStatusFilter(employmentStatus);
  const companyScope = getHrCompanyScopeKey();
  try {
    return await fetchJson(
      `${BASE_URL}/api/hr/employees?userId=${encodeURIComponent(userId)}&employmentStatus=${encodeURIComponent(statusFilter)}&companyScope=${encodeURIComponent(companyScope)}`,
    );
  } catch (error) {
    ...
    return {
      ...
      data: users
        ...
        .filter((user) => normalizeHrCompanyScopeKey(user.comp_name) === companyScope)
        .filter((user) => statusFilter === "all" || normalizeHrEmploymentStatusFilter(user.employment_status) === statusFilter)
...
async function fetchHrLeavesData(userId) {
  try {
    return await fetchJson(
      `${BASE_URL}/api/hr/leaves?userId=${encodeURIComponent(userId)}${getHrCompanyScopeQuery()}`,
    );
  } catch (error) {
    if (!isNotFoundError(error)) {
      throw error;
    ...
    return fetchJson(
      `${BASE_URL}/api/admin/leaves?adminId=${encodeURIComponent(adminId)}${getHrCompanyScopeQuery()}`,
    );
  }
}
...
async function fetchHrPayrollOverview(userId, monthKey) {
  try {
    return await fetchJson(
      `${BASE_URL}/api/hr/payroll/overview?userId=${encodeURIComponent(userId)}&month=${encodeURIComponent(monthKey)}${getHrCompanyScopeQuery()}`,
    );
  } catch (error) {
    if (!isNotFoundError(error)) {
      throw error;
    ...
    const adminId = await resolveHrFallbackAdminId();
    return fetchJson(
      `${BASE_URL}/api/payroll/admin/overview?adminId=${encodeURIComponent(adminId)}&month=${encodeURIComponent(monthKey)}${getHrCompanyScopeQuery()}`,
    );
  }
}
...
function calculateEmployeeScore(employee) {
  ...
  switch (normalizeHrAttendanceStatus(employee.attendance_status)) {
    case "present":
      score += 24;
      break;
    case "grace":
...
function renderAttendanceSection() {
  ...
  const lateBalance = Number.isFinite(Number(summary.lateBalance))
    ? Number(summary.lateBalance)
    : lateCount % 3;
  const lateLeaveEquivalent = Number.isFinite(Number(summary.lateLeaveEquivalent))
    ? Number(summary.lateLeaveEquivalent)
  ...
  renderMetricCardGrid("attendanceMetricGrid", [
    {
      ...
      note: "Completed full shift",
    },
    {
      label: "Grace",
      value: Number(summary.grace || 0),
      note: "Checked in within grace time",
    },
    {
      label: "Late",
      value: lateCount,
      note: "After grace limit",
    },
    {
      label: "Half Day",
      value: Number(summary.halfDay || 0),
      note: "Early check-out overrides late",
    },
    {
      label: "Pending",
      value: Number(summary.checkoutPending || 0),
      note: "Check-out still missing",
    },
    {
      label: "Absent",
      value: Number(summary.absent || 0),
      note: "Missing check-out or absent day",
    },
    {
      label: "Late = Leave",
      value: lateLeaveEquivalent,
      note: `${lateBalance} late pending, every 3 late = 1 leave`,
    },
  ]);

  renderHrSelfAttendanceControls();
    ...
    hrState.attendance.data.map(
      (row) => `
        <tr>
          ...
          <td>${escapeHtml(formatHrAttendanceTime(row.check_in))}</td>
          <td>${escapeHtml(formatHrAttendanceTime(row.check_out))}</td>
...
function getHrSelfAttendanceRow() {
  const currentUserId = Number(currentUser?.id || 0);
  return (
    hrState.attendance.data.find(
      (row) => Number(row.user_id || 0) === currentUserId,
    ) || null
  );
}

function renderHrSelfAttendanceControls() {
  const statusEl = document.getElementById("hrSelfAttendanceStatus");
  const actionsEl = document.getElementById("hrSelfAttendanceActions");
  if (!statusEl || !actionsEl) return;

  const row = getHrSelfAttendanceRow();
  const status = String(row?.status || row?.attendance_status || "not_marked");
  const hasCheckInToday = Boolean(row?.check_in);
  const hasCheckOutToday = hasValidHrAttendanceCheckout(row);
  const canCheckIn = !hasCheckInToday;
  const canCheckOut = hasCheckInToday && !hasCheckOutToday;
  const statusLabel = getAttendanceStatusLabel(status);
  const timeLabel = row?.check_in
    ? `Check in: ${formatHrAttendanceTime(row.check_in)}${row.check_out ? ` | Check out: ${formatHrAttendanceTime(row.check_out)}` : ""}`
    : "Location-based check-in is pending.";

  statusEl.textContent = `${statusLabel} | ${timeLabel}`;
  actionsEl.innerHTML = `
    <button type="button" class="ghost-btn hr-attendance-btn" onclick="markHrAttendance('check-in')" ${canCheckIn && !hrAttendanceUpdating ? "" : "disabled"}>
      <i class="fas fa-location-crosshairs"></i> Check In
    </button>
    <button type="button" class="ghost-btn hr-attendance-btn" onclick="markHrAttendance('check-out')" ${canCheckOut && !hrAttendanceUpdating ? "" : "disabled"}>
      <i class="fas fa-right-from-bracket"></i> Check Out
    </button>
  `;
}

async function markHrAttendance(type) {
  if (!currentUser?.id || hrAttendanceUpdating) return;
  ...
  try {
    hrAttendanceUpdating = true;
    setHrAttendanceButtonsDisabled(true);
    const location = await getHrCurrentLocation();
    ...
    if (!response.ok || !result.success) {
      throw new Error(result.message || result.error || "Attendance update failed");
    }

    showHrFeedback("Attendance", result.message || "Attendance updated", true);
    await refreshHrPanel({ showSuccessToast: false });
  } catch (error) {
    console.error("HR attendance update failed:", error);
    showHrFeedback("Attendance", error.message || "Attendance update failed", false);
  } finally {
    hrAttendanceUpdating = false;
    setHrAttendanceButtonsDisabled(false);
    renderHrSelfAttendanceControls();
  }
}

function setHrAttendanceButtonsDisabled(disabled) {
  document
    .querySelectorAll(".hr-attendance-btn")
    .forEach((button) => (button.disabled = disabled));
}

function showHrFeedback(title, message, isSuccess = true) {
  if (typeof window.showPopup === "function") {
    window.showPopup(title, message, isSuccess);
    return;
  }

  if (typeof showHrActionPopup === "function") {
    showHrActionPopup(title, message, isSuccess);
    return;
  }
        ...
        const message =
          error.code === error.PERMISSION_DENIED
            ...
            : error.code === error.TIMEOUT
              ? "Location fetch timed out. GPS/location on karke retry karo."
...
function renderPayrollSection() {
  ...
  const generatedRows = payrollRows.filter((row) => row.isGenerated || row.payrollId);
  const totalMonthlyPayout = payrollRows.reduce(
  ...
  renderMetricCardGrid("payrollMetricGrid", [
    {
      ...
      value: generatedRows.length,
      note: `Generated out of ${payrollRows.length} employees`,
    },
    {
...
function renderEmployeeStatusCards(employees) {
  const statusFilterLabel =
    normalizeHrEmploymentStatusFilter(hrState.employeeStatusFilter) === "inactive"
      ? "deactive"
      : normalizeHrEmploymentStatusFilter(hrState.employeeStatusFilter) === "all"
        ? "all"
        : "active";
  ...
  renderMetricCardGrid("employeeStatusGrid", [
    { label: "Employee Directory", value: employees.length, note: `${statusFilterLabel} employees in current filter` },
...
function renderHrEmployeeAccessBadge(employee = {}) {
  const isActive = isHrEmployeeActive(employee);
  const detail = isActive
    ? "Login allowed"
    : employee.deactivated_at
      ? `Blocked ${formatDate(employee.deactivated_at)}`
      : "Login blocked";

  return `
    <div class="hr-access-stack">
      ${statusPill(isActive ? "Active" : "Deactive", isActive ? "good" : "bad")}
      <small>${escapeHtml(detail)}</small>
    </div>
  `;
}

function renderHrEmployeeAccessAction(employee = {}) {
  const employeeId = Number(employee.id || 0);
  const isActive = isHrEmployeeActive(employee);
  const isSelf = employeeId && Number(currentUser?.id || 0) === employeeId;
  ...
  return `
    <button
      type="button"
      class="profile-action-btn ${isActive ? "employee-deactivate-btn" : "employee-activate-btn"}"
      onclick="toggleHrEmployeeStatus(${employeeId}, '${nextStatus}')"
    ...
    employees.map((employee) => {
      ...
      return `
        <tr>
          ...
          <td>${renderHrEmployeeAccessBadge(employee)}</td>
          <td>
            ...
            ${renderHrEmployeeAccessAction(employee)}
          </td>
        </tr>
      `;
    }),
...
async function toggleHrEmployeeStatus(employeeId, nextStatus) {
  const employee = hrState.employees.find((item) => String(item.id) === String(employeeId));
  const normalizedStatus = normalizeHrEmploymentStatusFilter(nextStatus, "");

  if (!employee || !["active", "inactive"].includes(normalizedStatus)) {
    showToast("Invalid employee status request.", true);
    return;
  }

  if (
    normalizedStatus === "inactive" &&
    !window.confirm(`Deactivate ${employee.name || "this employee"}? The employee will no longer be able to login.`)
  ) {
    return;
  }

  try {
    const response = await fetch(`${BASE_URL}/api/admin/users/${Number(employeeId)}/employment-status`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    showToast(data.message || "Employee status updated.");
    await refreshHrPanel({ showSuccessToast: false });
  } catch (error) {
    console.error("HR employee status update failed:", error);
    showToast(error.message || "Failed to update employee status.", true);
  }
}
