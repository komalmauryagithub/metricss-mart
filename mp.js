...
const LOGIN_API_URLS = [`${BASE_URL}/api/login`, `${BASE_URL}/login`];
...
function applyLoginCompanyTheme(companyKey) {
  ...
  document.querySelectorAll(".hero-logo").forEach((logo) => {
    logo.src = "logo_.png";
    logo.alt = "Metrics Mart";
  });
  document
    .querySelectorAll("link[rel~='icon']")
    .forEach((icon) => icon.setAttribute("href", "logo_.png"));
}
...
async function fetchWithRouteFallback(urls, options, routeLabel) {
  let lastError = null;
  ...
  for (const url of urls) {
    try {
      ...
      if (response.status === 404) {
        lastError = new Error(`${routeLabel} route missing at ${url}`);
        continue;
      }
  ...
  throw lastError || new Error(`${routeLabel} route is not available`);
}
...
function getRedirectPage(role, companyKey = "") {
  ...
  switch ((role || '').toLowerCase().trim()) {
    ...
    case 'email-marketing':
    case 'emailmarketing':
      return `email-marketing.html${suffix}`;
    case 'me':
...
document.getElementById("loginFormElement").addEventListener("submit", async function (e) {
  ...
  try {
    ...
    const { response: res, data } = await fetchWithRouteFallback(LOGIN_API_URLS, {
      method: "POST",
      headers: {
    ...
    showPopup("Error", error.message || "Server error. Please try again.", false);
  } finally {
    btn.innerHTML = originalText;
    btn.disabled = false;
  }
