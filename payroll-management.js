(function () {
  ...
  const FIXED_PAYROLL_OTHER_TAX = 200;
  ...
  function getCurrentUser() {
    ...
    function getSelectedCompanyScope() {
    const user = getCurrentUser();
    const role = user ? String(user.role || "").toLowerCase().trim() : "";
    
    if (role === "admin" && user) {
        return user.company_scope || user.company_key || user.selected_company || user.comp_name || "metrics";
    }

    if (typeof window.getAdminPanelCompanyScope === "function") {
      return window.getAdminPanelCompanyScope();
    }

    const stored = String(localStorage.getItem("adminSelectedCompanyScope") || "").trim();
    return stored || "";
  }
  ...
  function normalizeRole(role) {
    const normalized = String(role || "")
      ...
      .toLowerCase()
      .replace(/[\s-]+/g, "_");
    return normalized === "emailmarketing" ? "email_marketing" : normalized;
  }

  function canUseSalesCompensation(role) {
    return ["me", "tme", "email_marketing"].includes(normalizeRole(role));
  }
  ...
  function calculateSalarySplit(totalSalary) {
    const grossSalary = Number(totalSalary || 0);
    const basicSalary = Number((grossSalary * 0.55).toFixed(2));
    const hraAmount = Number((grossSalary * 0.27).toFixed(2));
    const allowanceAmount = Number((grossSalary - basicSalary - hraAmount).toFixed(2));
  ...
  // Salary slip ke liye unlock date check:
  // Kisi bhi month ka payslip sirf next month ke 12th se available hoga.
  // Example: June (2026-06) ka payslip → 12 July 2026 se available.
  function isPayslipUnlocked(monthKey) {
    if (!monthKey) return false;
    
    // Check if user is admin or hr - they can bypass the date lock
    const currentUser = typeof getCurrentUser === "function" ? getCurrentUser() : null;
    const role = currentUser && currentUser.role ? String(currentUser.role).toLowerCase().trim() : "";
    if (role === "admin" || role === "hr") return true;

    const [year, month] = String(monthKey).split("-").map(Number);
    if (!year || !month) return false;
    // Next month ka 12th din
    const unlockDate = new Date(year, month, 12); // month is 0-indexed, so `month` (not month-1) = next month
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    return today >= unlockDate;
  }

  function payslipUnlockDateLabel(monthKey) {
    if (!monthKey) return "";
    const [year, month] = String(monthKey).split("-").map(Number);
    if (!year || !month) return "";
    const unlockDate = new Date(year, month, 12);
    return unlockDate.toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" });
  }
      ...
      <div class="payroll-shell">
        ...
        <section class="payroll-toolbar">
          ...
          <div class="payroll-toolbar-actions">
            ...
            <button type="button" class="payroll-btn primary" id="payrollAdminGenerateAllBtn">Generate All</button>
          </div>
        </section>
  ...
  function attachAdminEvents() {
    ...
    const generateAllBtn = document.getElementById("payrollAdminGenerateAllBtn");
    generateAllBtn?.addEventListener("click", () => {
      generateAdminPayroll("all", null, generateAllBtn);
    });
    ...
    tableBody?.addEventListener("click", async (event) => {
      ...
      if (action === "save") {
        ...
        await generateAdminPayroll("single", employeeId, button);
    ...
    tableBody?.addEventListener("input", (event) => {
      const field = event.target.closest('[data-field="salary"]');
      if (!field) return;
  ...
  function updateAdminCompensationRow(row) {
    ...
    const salarySplit = calculateSalarySplit(isCommission ? 0 : salaryInput?.value || 0);
    ...
    row?.querySelector('[data-split-field="basic"]')?.replaceChildren(
      document.createTextNode(formatCurrency(salarySplit.basicSalary)),
    );
    row?.querySelector('[data-split-field="hra"]')?.replaceChildren(
      document.createTextNode(formatCurrency(salarySplit.hraAmount)),
    );
    row?.querySelector('[data-split-field="allowance"]')?.replaceChildren(
      document.createTextNode(formatCurrency(salarySplit.allowanceAmount)),
    );
  }
  ...
  function buildAdminOverviewUrl() {
    ...
    const companyScope = getSelectedCompanyScope();
    if (companyScope) {
      params.set("companyScope", companyScope);
  ...
  async function loadAdminPayrollSection(forceRefresh) {
    ...
    try {
      ...
      state.adminRows = Array.isArray(payload.data)
        ? payload.data.filter((row) => normalizeRole(row.role) !== "admin")
        : [];
    ...
    tbody.innerHTML = rows
      .map((row) => {
        ...
        const grossSalary = Number(
          row.grossSalary ||
            row.salary ||
            (Number(row.basicSalary || 0) + Number(row.hraAmount || 0) + Number(row.allowanceAmount || 0)) ||
            0,
        );
        const salarySplit = calculateSalarySplit(isCommission ? 0 : grossSalary);
        const professionalTax = FIXED_PAYROLL_OTHER_TAX;
        const pfDeduction = Number(row.pfDeduction || 0);
        const tdsDeduction = Number(row.tdsDeduction || 0);
        const totalSalaryEarnings = Number(row.totalSalaryEarnings || grossSalary);
        const totalEarnings = Number(
          (totalSalaryEarnings + Number(row.bonusAmount || 0) + Number(row.incentiveAmount || 0)).toFixed(2),
        );
          ...
          <tr data-employee-id="${Number(row.employeeId)}" data-role="${escapeHtml(normalizedRole)}">
            ...
            <td>
              <div class="payroll-cell-stack">
                ...
                <label data-comp-section="salary" class="${isCommission ? "hidden" : ""}">
                  ...
                  <input class="payroll-inline-input" data-field="salary" type="number" min="0" step="0.01" value="${escapeHtml(grossSalary.toFixed(2))}" ${isCommission ? "disabled" : ""} />
                </label>
                <div class="payroll-split-grid" data-comp-section="salary">
                  <div class="payroll-split-row">
                    <span>
                      <strong>Basic Salary</strong>
                      <small>Total Salary x 55%</small>
                    </span>
                    <b data-split-field="basic">${escapeHtml(formatCurrency(salarySplit.basicSalary))}</b>
                  </div>
                  <div class="payroll-split-row">
                    <span>
                      <strong>HRA</strong>
                      <small>Total Salary x 27%</small>
                    </span>
                    <b data-split-field="hra">${escapeHtml(formatCurrency(salarySplit.hraAmount))}</b>
                  </div>
                  <div class="payroll-split-row">
                    <span>
                      <strong>Allowance</strong>
                      <small>Total Salary x 18%</small>
                    </span>
                    <b data-split-field="allowance">${escapeHtml(formatCurrency(salarySplit.allowanceAmount))}</b>
                  </div>
                </div>
                ${canUseCommission ? `
                ...
                ${isCommission
                  ...
                  : hasAutoTargetIncentive
                    ? `<small data-comp-section="salary">Target ${escapeHtml(formatCurrency(salaryTarget))} (${escapeHtml(formatCurrency(grossSalary))} x 7)</small>
            ...
            <td>
              <div class="payroll-cell-stack">
                ...
                <small>Late Marks: <strong>${Number(row.lateMarks || 0)}</strong></small>
                <small>Leave Days: <strong>${Number(row.leaveEquivalentDays || row.totalLeaveDays || 0)}</strong></small>
                <small>Month Days: <strong>${Number(row.workingDays || 0)}</strong></small>
            ...
            <td>
              <div class="payroll-cell-stack">
                ...
                <label>
                  <span>Other Tax</span>
                  <input class="payroll-inline-input" data-field="professional-tax" type="number" min="0" step="0.01" value="${escapeHtml(professionalTax.toFixed(2))}" readonly />
                </label>
                <label>
                  <span>PF Amount</span>
                  <input class="payroll-inline-input" data-field="pf-deduction" type="number" min="0" step="0.01" value="${escapeHtml(pfDeduction.toFixed(2))}" />
                </label>
                <label>
                  <span>TDS Amount</span>
                  <input class="payroll-inline-input" data-field="tds-deduction" type="number" min="0" step="0.01" value="${escapeHtml(tdsDeduction.toFixed(2))}" />
                </label>
              </div>
            </td>
            <td>
              <div class="payroll-cell-stack">
                <small>Salary Earnings: <strong>${escapeHtml(formatCurrency(totalSalaryEarnings))}</strong></small>
                <small>Total Earnings: <strong>${escapeHtml(formatCurrency(totalEarnings))}</strong></small>
                <small>Total Deductions: <strong>${escapeHtml(formatCurrency(totalDeductions))}</strong></small>
                <small style="color:#0f766e;font-weight:600;">Net Payable: <strong>${escapeHtml(formatCurrency(row.finalSalary || 0))}</strong></small>
            ...
            <td>
              <div class="payroll-row-actions">
                ...
                <button type="button" class="payroll-btn primary" data-action="generate">Generate</button>
                <button
                  type="button"
  ...
  function collectRowPayload(employeeId) {
    ...
    return {
      ...
      professionalTax: String(FIXED_PAYROLL_OTHER_TAX),
      pfDeduction: row.querySelector('[data-field="pf-deduction"]')?.value || "0",
      tdsDeduction: row.querySelector('[data-field="tds-deduction"]')?.value || "0",
  ...
  async function generateAdminPayroll(mode, employeeId, button) {
    ...
    // Payroll generate karne ke liye bhi unlock check:
    // Month complete hone ke baad aur next month ke 12th se pehle generate nahi hoga
    if (!isPayslipUnlocked(month)) {
      showMessage(
        "Payroll Locked",
        `${formatMonthLabel(month)} payroll will be available for generation after ${payslipUnlockDateLabel(month)}.`,
        false,
      );
      return;
    }
  ...
  async function loadEmployeePayrollSection(forceRefresh) {
    ...
    try {
      ...
      if (payload.preview?.payrollId && isPayslipUnlocked(month)) {
        payslipBtn.disabled = false;
        payslipBtn.dataset.payrollId = String(payload.preview.payrollId);
        payslipBtn.title = "";
        payslipBtn.textContent = "Download Current Payslip";
      } else if (payload.preview?.payrollId && !isPayslipUnlocked(month)) {
        payslipBtn.disabled = true;
        payslipBtn.dataset.payrollId = "";
        payslipBtn.textContent = `🔒 Available from ${payslipUnlockDateLabel(month)}`;
        payslipBtn.title = `Salary for ${formatMonthLabel(month)} will be released on ${payslipUnlockDateLabel(month)}`;
      } else {
        payslipBtn.disabled = true;
        payslipBtn.dataset.payrollId = "";
        payslipBtn.textContent = "Download Current Payslip";
  ...
  function renderEmployeeSummary(preview, employee) {
    ...
    const grossAmount = Number(preview.grossSalary || employee.salary || 0);
    const basicAmount = Number(preview.basicSalary || 0);
    const hraAmount = Number(preview.hraAmount || 0);
    const allowanceAmount = Number(preview.allowanceAmount || 0);
    const totalDeductions = Number(
      preview.totalDeductions != null
        ? preview.totalDeductions
        : FIXED_PAYROLL_OTHER_TAX +
            Number(preview.pfDeduction || 0) +
            Number(preview.tdsDeduction || 0) +
            Number(preview.leaveDeduction || 0) +
            Number(preview.penaltyAmount || 0),
    );
    const earnedSalaryAmount = Number(preview.finalSalary || 0);

    container.innerHTML = `
      <article class="payroll-summary-tile">
        <span>${isCommission ? "Commission Rate" : "Gross Salary"}</span>
        <strong>${isCommission ? `${commissionPercent.toFixed(0)}%` : escapeHtml(formatCurrency(grossAmount))}</strong>
      </article>
      <article class="payroll-summary-tile">
        <span>Basic Salary (55%)</span>
        <strong>${escapeHtml(formatCurrency(basicAmount))}</strong>
      </article>
      <article class="payroll-summary-tile">
        <span>HRA (27%)</span>
        <strong>${escapeHtml(formatCurrency(hraAmount))}</strong>
      </article>
      <article class="payroll-summary-tile">
        <span>Allowance (18%)</span>
        <strong>${escapeHtml(formatCurrency(allowanceAmount))}</strong>
      </article>
      <article class="payroll-summary-tile">
        <span>Late Marks</span>
        <strong>${Number(preview.lateMarks || 0)}</strong>
      </article>
      <article class="payroll-summary-tile">
      ...
      <article class="payroll-summary-tile">
        ...
        <strong>${Number(preview.totalLeaveDays != null ? preview.totalLeaveDays : (preview.unpaidLeaveDays || 0))}</strong>
      </article>
      <article class="payroll-summary-tile">
        <span>Half Days</span>
      ...
      <article class="payroll-summary-tile">
        ...
        <strong>${escapeHtml(formatCurrency(totalDeductions))}</strong>
      </article>
      <article class="payroll-summary-tile">
      ...
      <article class="payroll-summary-tile earned-salary-tile">
        <span>Net Payable</span>
        <strong>${escapeHtml(formatCurrency(earnedSalaryAmount))}</strong>
      </article>
      <article class="payroll-summary-tile">
        <span>Status</span>
  ...
  function renderEmployeeBreakdown(preview) {
    ...
    container.innerHTML = `
      <ul class="payroll-inline-list">
        <li><span>Gross Salary</span><strong>${escapeHtml(formatCurrency(preview.grossSalary || 0))}</strong></li>
        <li><span>Basic Salary (55%)</span><strong>${escapeHtml(formatCurrency(preview.basicSalary || 0))}</strong></li>
        <li><span>HRA (27%)</span><strong>${escapeHtml(formatCurrency(preview.hraAmount || 0))}</strong></li>
        <li><span>Allowance (18%)</span><strong>${escapeHtml(formatCurrency(preview.allowanceAmount || 0))}</strong></li>
        ...
        <li><span>Late Marks</span><strong>${Number(preview.lateMarks || 0)}</strong></li>
        <li><span>Other Tax</span><strong>${escapeHtml(formatCurrency(FIXED_PAYROLL_OTHER_TAX))}</strong></li>
        <li><span>PF Deduction</span><strong>${escapeHtml(formatCurrency(preview.pfDeduction || 0))}</strong></li>
        <li><span>TDS Deduction</span><strong>${escapeHtml(formatCurrency(preview.tdsDeduction || 0))}</strong></li>
        <li><span>Leave Deduction</span><strong>${escapeHtml(formatCurrency(preview.leaveDeduction || 0))}</strong></li>
  ...
  function renderEmployeeHistory(history) {
    ...
    tbody.innerHTML = history
      .map(
        (row) => {
          const unlocked = isPayslipUnlocked(row.monthKey);
          const lockLabel = unlocked ? "" : `🔒 From ${payslipUnlockDateLabel(row.monthKey)}`;
          const lockTitle = unlocked ? "" : `Salary for ${formatMonthLabel(row.monthKey)} will be released on ${payslipUnlockDateLabel(row.monthKey)}`;
          return `
          <tr>
            <td data-label="Month">${escapeHtml(formatMonthLabel(row.monthKey))}</td>
            <td data-label="Gross Salary">${escapeHtml(formatCurrency(row.grossSalary || row.basicSalary || 0))}</td>
            <td data-label="Deduction">${escapeHtml(formatCurrency(row.totalDeductions != null ? row.totalDeductions : ((row.leaveDeduction || 0) + (row.penaltyAmount || 0))))}</td>
            <td data-label="Bonus + Incentive">${escapeHtml(formatCurrency((row.bonusAmount || 0) + (row.incentiveAmount || 0)))}</td>
            <td data-label="Final Salary">${escapeHtml(formatCurrency(row.finalSalary || 0))}</td>
            <td data-label="Status">${escapeHtml(String(row.status || "generated").toUpperCase())}</td>
            <td data-label="Payslip">
              ${unlocked
                ? `<button type="button" class="payroll-btn ghost" data-payroll-id="${Number(row.payrollId || 0)}">Download</button>`
                : `<span class="payroll-btn ghost" style="cursor:not-allowed;opacity:0.6;" title="${escapeHtml(lockTitle)}">${escapeHtml(lockLabel)}</span>`
              }
            </td>
          </tr>
        `;
  ...
  function renderEmployeeChart(preview) {
    ...
    state.employeeChart = new window.Chart(canvas.getContext("2d"), {
      type: "doughnut",
      data: {
        labels: ["Net Salary", "Total Deductions", "Bonus + Incentive"],
        datasets: [
          {
            data: [
              Number(preview.finalSalary || 0),
              chartDeductions,
              Number(preview.bonusAmount || 0) + Number(preview.incentiveAmount || 0),
            ],
  ...
  function downloadPayslip(payrollId) {
    ...
    window.open(
      `${BASE_URL}/api/payroll/payslip/${payrollId}?requesterId=${user.id}&v=${Date.now()}`,
      "_blank",
      "noopener,noreferrer",
    );
  }
